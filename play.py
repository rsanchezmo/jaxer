...
if __name__ == '__main__':
    ...
    if agent.config.dataset_mode == 'synthetic':
        dataset = jaxer.utils.SyntheticDataset(
            config=jaxer.config.SyntheticDatasetConfig.from_dict(agent.config.synthetic_dataset_config))
    ...
    if plot_predictions:
        ...
        if agent.config.dataset_mode == 'synthetic':
            for i in range(30):
                ...
                y_pred = agent(x)
                metrics = jaxer.utils.plot_predictions(x=x, y_true=y_true, y_pred=y_pred, normalizer=normalizer,
